# Contributing to {{cookiecutter.project_name|replace('-', ' ')|title }}
...
To push your new project to GitHub, first create a repository named `{{cookiecutter.project_name}}` on GitHub, then:
...
# This creates: {{cookiecutter.project_name}}-v0.1.0.tar.gz
```
...
def main() -> None:
    ...
    subprocess.run(["python", "-m", "compileall", "{{cookiecutter.project_slug}}"], check=True)
...
- Check that `requirements.txt` has been generated

**"ERROR: Failed to compile requirements.txt"**
- Run `just install` to set up UV properly
- Verify `pyproject.toml` has valid dependencies

**Archive too large**
- Check what's included: `tar -tzf {{cookiecutter.project_name}}-v*.tar.gz`
