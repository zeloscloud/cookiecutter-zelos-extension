...
PUBLISHER_ID_REGEX = r"^[a-z0-9]+(?:-[a-z0-9]+)*$"
publisher_id = "{{cookiecutter.publisher_id}}"
if not re.match(PUBLISHER_ID_REGEX, publisher_id):
    print(
        f"ERROR: The publisher id '{publisher_id}' must be lowercase alphanumeric with optional hyphens (e.g. 'acme-labs')."
    )
    sys.exit(1)

EXTENSION_ID_REGEX = r"^[a-z0-9]+(?:-[a-z0-9]+)*\.[a-z0-9]+(?:-[a-z0-9]+)*$"
extension_id = "{{cookiecutter.extension_id}}"
if not re.match(EXTENSION_ID_REGEX, extension_id):
    print(
        f"ERROR: The extension id '{extension_id}' must follow 'publisher.extension-name' in lowercase with hyphens."
    )
    sys.exit(1)
